...
public class totalCommander {
	...
	static final String VERSION="FileFox commander (вер. 1.09.28.10)";
...
//***********************************END MENU*******************************************************	
	...
	private String newPathL="C:\\";
	private String newPathR="C:\\";
	private static File fileLeft;
	private static Path PathfileLeft;
	private static Path PathfileRight;
...
private void createFSL(String paths) {//**************************************LEFT PANEL
	...
	try {
		fileLeft=new File(paths).getCanonicalFile();	
	}catch(IOException e) {e.printStackTrace();}
	
	PathfileLeft=Paths.get(paths);
...
//**********************************CREATE ROOT ... in ARRAYS************************************************************
	...
	Date dateFile;
	String date;
	//JAVA.NIO формируем символичесские ссылки
	try (DirectoryStream<Path> dirStream=Files.newDirectoryStream(PathfileLeft) )
			{
		for(Path dirsAndFile:dirStream) {
			fSize=dirsAndFile.toFile().length();//размер в байтах
			fTime=dirsAndFile.toFile().lastModified();// Последнее время изменения файла
			dateFile=new Date(fTime);
			date=dateFormat.format(dateFile);
			
			BasicFileAttributes attrNIO=Files.readAttributes(dirsAndFile,BasicFileAttributes.class);
			
			if(Files.isSymbolicLink(dirsAndFile)) {
				listFileL.add(dirsAndFile.toFile()); //Полный путь к файлу
				fsViewL[x]=new FileView("["+dirsAndFile.getFileName()+"]","LINK","ссылка",String.valueOf(date));
				x++;
				countDirL++;
			}
			else if(attrNIO.isDirectory()) {
				
				listFileL.add(dirsAndFile.toFile()); //Полный путь к файлу
				fsViewL[x]=new FileView("["+dirsAndFile.getFileName()+"]","DIR","папка",String.valueOf(date));
				x++;
				countDirL++;
				}
		}
	}catch(ArrayIndexOutOfBoundsException error) {System.out.println(error);
	}catch(InvalidPathException e1) {e1.printStackTrace();
	}catch(NotDirectoryException e2) {e2.printStackTrace();
...
private void createFSR(String paths) {//RIGHTS PANEL**********************************
	...
	try {
		fileRight=new File(paths).getCanonicalFile();	
	}catch(IOException e) {e.printStackTrace();}
	
	PathfileRight=Paths.get(paths);
...
//***********************JAVA NIO***************************************
	//JAVA.NIO формируем символичесские ссылки
	try (DirectoryStream<Path> dirStream=Files.newDirectoryStream(PathfileRight) )
			{
		for(Path dirsAndFile:dirStream) {
			fSize=dirsAndFile.toFile().length();//размер в байтах
			fTime=dirsAndFile.toFile().lastModified();// Последнее время изменения файла
			...
			if(Files.isSymbolicLink(dirsAndFile)) {
				listFileR.add(dirsAndFile.toFile()); //Полный путь к файлу
				fsViewR[x]=new FileView("["+dirsAndFile.getFileName()+"]","LINK","ссылка",String.valueOf(date));
				x++;
				countDirR++;
			}
			else if(attrNIO.isDirectory()) {
				...
				listFileR.add(dirsAndFile.toFile()); //Полный путь к файлу
				fsViewR[x]=new FileView("["+dirsAndFile.getFileName()+"]","DIR","папка",String.valueOf(date));
				x++;
				countDirR++;
				}
		}
...
private String getCutPathFile(String f) {
	String path=f;
	System.out.println("Проверка нулевого пути в начале path="+path);
...
//*********************************Блок кода с нажатием на  ROOT ... ЛЕВАЯ Таблица************************************************				
		...
		if(index<fsViewL.length && fsViewL[index].getType()=="DIR") {
		...
		if(index<fsViewL.length && fsViewL[index].getType()=="LINK") {
			System.out.println("УРААА, Символичесская ссылка!!!!");
			Path pathNIO=Paths.get(listFileL.get(index).getAbsolutePath());
			System.out.println(pathNIO);
			
			try {
				newPathL=Files.readSymbolicLink(pathNIO).toString();
				createFSL(newPathL);
				updateTable("L");
			}catch(IOException e) {e.printStackTrace();}
		}
...
//******************Вырезание одного уровня из пути к файлу**************************************************************
		...
		if(index<fsViewR.length && fsViewR[index].getType()=="DIR") {
			System.out.println("УРААА, ПАПКА!!!!");
		...
		if(index<fsViewR.length && fsViewR[index].getType()=="LINK") {
			System.out.println("УРААА, Символичесская ссылка!!!!");
			Path pathNIO=Paths.get(listFileR.get(index).getAbsolutePath());
			System.out.println(pathNIO);
			try {
				newPathR=Files.readSymbolicLink(pathNIO).toString();
				createFSR(newPathR);
				updateTable("R");
			}catch(IOException e) {e.printStackTrace();}
		}
